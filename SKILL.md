...
description: Research keyword ideas, search volume, competition, and keyword-planning metrics with the Google Ads API KeywordPlanIdeaService. Use for Google Ads keyword research, SEO/SEM keyword discovery, Keyword Planner-style keyword ideas, Vietnamese keyword research using Google Ads data, or generating keyword CSV/JSON reports from seed keywords, URLs, locations, languages, and Google Search network settings.
---
...
1. Collect inputs: customer ID, seed keywords and/or URL/site, target language, target geo IDs, network, adult keyword setting, and desired output format.
...
5. Mention any limits or missing inputs, especially when location/language targeting is broad or credentials were not available.
...
If the user does not specify targeting, ask only when the ambiguity materially changes the work. Otherwise use conservative defaults and state them:

- Vietnam: language ID `1019` for Vietnamese, geo target `2704` for Vietnam.
- United States: language ID `1000` for English, geo target `2840` for United States.
- Network: `GOOGLE_SEARCH_AND_PARTNERS` unless the user asks for Google Search only.
- Adult keywords: exclude by default.

Use Google Ads resource IDs, not country names, in API requests. If IDs are uncertain or current precision matters, verify in Google Ads geo/language constants before running.
